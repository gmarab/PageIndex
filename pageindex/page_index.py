...
async def check_title_appearance_in_start_concurrent(structure, page_list, model=None, logger=None):
    ...
    results = await gather_with_limit(tasks, return_exceptions=True)
...
async def extract_toc_content(content, model=None):
    ...
    response, finish_reason = await ChatGPT_API_with_finish_reason(model=model, prompt=prompt)
...
async def toc_transformer(toc_content, model=None):
    ...
    while not (if_complete == "yes" and finish_reason == "finished"):
        ...
        new_complete, finish_reason = await ChatGPT_API_with_finish_reason(model=model, prompt=prompt)
...
async def add_page_number_to_toc(part, structure, model=None):
    ...
    current_json_raw = await ChatGPT_API(model=model, prompt=prompt)
...
def page_index_main(doc, opt: config):
